...
export function AdminSettingsPage() {
  ...
  async function load() {
    const [s, b] = await Promise.all([api.get<SiteSettings>('/settings'), api.get<Banner[]>('/banners')]);
    setSettings(s);
    setBanners(b);
  }
  ...
  async function handleUpdateBanner(id: number, data: Partial<Pick<Banner, 'title' | 'subtitle'>>) {
    const updated = await api.patch<Banner>(`/banners/${id}`, data);
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          <div className="bg-card-subtle rounded-2xl p-4">
            ...
            <ImageUploadButton spec={IMAGE_SPECS.logo} buttonLabel="Trocar logo" onUpload={handleUploadLogo} />
          </div>
          ...
          <div className="bg-card-subtle rounded-2xl p-4">
            ...
            <ImageUploadButton spec={IMAGE_SPECS.hero} buttonLabel="Trocar imagem" onUpload={handleUploadHero} />
          </div>
        </div>

        <div>
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-display uppercase tracking-wider text-lg text-text-main">Banners do Carrossel</h2>
            <ImageUploadButton
              spec={IMAGE_SPECS.banner}
              buttonLabel="Adicionar banner"
              onUpload={handleUploadBanner}
              className="text-sm rounded-full bg-primary hover:bg-primary-hover text-white px-4 py-1.5 transition disabled:opacity-60"
            />
          </div>
              ...
              <div key={b.id} className="relative bg-card-subtle rounded-xl overflow-hidden">
                ...
                <div className="p-3 flex flex-col gap-1.5">
                  <input
                    ...
                    onBlur={(e) => e.target.value !== (b.title ?? '') && handleUpdateBanner(b.id, { title: e.target.value })}
                    className="text-sm rounded-lg border border-border px-2 py-1 outline-none focus:border-primary"
                  />
                  <input
                    defaultValue={b.subtitle ?? ''}
                    placeholder="Subtítulo (opcional)"
                    onBlur={(e) =>
                      e.target.value !== (b.subtitle ?? '') && handleUpdateBanner(b.id, { subtitle: e.target.value })
                    }
                    className="text-xs rounded-lg border border-border px-2 py-1 outline-none focus:border-primary"
                  />
                </div>
              </div>
            ))}
